...
class Dark_launch
{
  ...
  /**
  * Initializes library
  * @param $user string - The name of the user who is dark launching
  * @param $project string - The name of the project which is dark launching
  * @param $redis obj - The instance of redis you wish to use
  */
  ...
  */
  ...
  {
    log_error(['status' => '404', 'error' => "No dark launch value exists for: {$feature}"]);
    return FALSE;
  }
  ...
  */
  ...
  {
    $this->CI->load->config('dark_launch', FALSE, TRUE);
    $features = $this->CI->config->item('dark_launch_features');
  ...
  */
  private function _parse_time($feature)
  {
    ...
    if(!is_null($feature['stop']) AND ($feature['stop'] < $feature['start'])){
      log_error('Invalid value for stop time.');
      return FALSE;
    }
  ...
  */
  private function _parse_percentage($feature)
  {
    ...
    if($percentage < 0 OR $percentage > 100) {
      log_error('Dark launch percentage is not in the 0 - 100 range.');
      return FALSE;
    }
